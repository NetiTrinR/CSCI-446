			...
			try:
				...
				if intf_a.next_avail_time <= time.time():
					...
					print('%s: transmitting packet "%s" on %s %s -> %s, %s \n' \
						...
						' - queue size %d ' \
						% (self, pkt_S, node_a, node_a_intf, node_b, node_b_intf, intf_a.next_avail_time - time.time(), intf_a.out_queue.qsize()), end="")
					for i in node_a.pkt_queue_stat:
						print('priority %d: %d packets, ' % (i, node_a.pkt_queue_stat[i]), end="")
					print()
